        ...
        chrome.storage.local.get(['ColorPalette'], function (res) {
          ...
          if (prevData.length > 0) {
            chrome.storage.local.set({
              'ColorPalette': JSON.stringify(prevData)
            });
          }
        });
      });
...
chrome.runtime.onMessage.addListener(function (message, sender, response) {
  if (message.command == 'login') {
    ...
    const auth = getAuth();
    signInWithEmailAndPassword(auth, message.data.email, message.data.password).then(userCredential => {
    ...
    const auth = getAuth();
    createUserWithEmailAndPassword(auth, message.data.email, message.data.password).then(userCredential => {
      const user = userCredential.user;
      if (user) response({
        type: 'auth',
        status: 'success',
